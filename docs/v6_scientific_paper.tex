% Scientific Paper v6: The Agentic Tumor Board (Comprehensive Edition)
% Expanded to 8+ pages with detailed Case Study, Algorithmic Formalization, Related Work, and Visual Evidence
% January 2026
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small \textit{The Agentic Tumor Board (V6.0)}}
...
This paper presents the \textbf{V6 Virtual Tumor Board}, an Agentic System designed explicitly for the high-stakes, low-resource context of Indian oncology. Our contributions are:
...
\end{tikzpicture}
\caption{V6 Hybrid Architecture. Note the MARC-v1 Loop in the top-left (Data Integrity) and the Adversarial Links in the bottom (Safety/Cost).}
...
Figure \ref{fig:biomarkers} illustrates the precise extraction of biomarkers from clinical reports, highlighting how the system flags critical information like hormone receptor status which is fundamental for the downstream decision making by Dr. Chikitsa (Medical Oncologist).

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{screenshot-biomarkers.png}
    \caption{\textbf{Agentic Data Extraction.} The system reliably parses unstructured reports to extract structured biomarkers (ER/PR/HER2), utilizing the MARC-v1 evaluator loop to verify accuracy against source text.}
...
This is the core innovation of V6. Unlike "collaborative" multi-agent systems, this engine is designed for **conflict**.

\subsubsection{The Chain of Debate}
The Moderator orchestrates a multi-turn game (Figure \ref{fig:case_ui}), where agents deliberate in real-time.
...
To illustrate the system's "reasoning trace," we present a simulated case. Figure \ref{fig:hero} shows the system's entry point, emphasizing the "Human-in-the-loop" design where patients or junior doctors initiate the process.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{screenshot-hero.png}
    \caption{\textbf{System Entry Point.} The landing page guides users (patients or clinicians) to upload heterogeneous data sources, initiating the agentic workflow.}
    \label{fig:hero}
\end{figure}
