...
\hypersetup{
    ...
    pdfauthor={Ashish Makani, Anurag Agrawal}
}
...
% ============================================================================
% TITLE AND AUTHORS (medRxiv format)
% ============================================================================
\title{
    \vspace{-1cm}
...
\author[1,*]{Ashish Makani}
\author[1]{Anurag Agrawal}

\affil[1]{Koita Centre for Digital Health - Ashoka (KCDH-A), Ashoka University, Sonipat, Haryana, India}
...
Email: \texttt{spiff007@gmail.com}
...
Our system integrates three core components: (1) \textbf{MARC-v1} reliability loops using evaluator-optimizer patterns to improve extraction fidelity; (2) \textbf{MAI-DxO} adversarial deliberation designed to prevent sycophantic consensus through role-based prompting and domain authority veto mechanisms; and (3) \textbf{MedGemma} multimodal integration for anchoring clinical discussions in imaging evidence.

We present preliminary observations from 10 clinically diverse synthetic cases spanning genomic complexity (KRAS G12C+ NSCLC), financial constraints (rural HER2-equivocal breast cancer), and rare presentations (pediatric GBM with H3 G34R). \textbf{These observations are from informal developer testing and have not been validated through rigorous clinical evaluation.} The Stewardship Agent demonstrates the ability to surface biosimilar alternatives and cost considerations relevant to Indian healthcare contexts.

We propose that treating tumor boards as \textit{scientific simulations} rather than conversations---decoupling data ingestion from deliberation, enforcing adversarial critique, and grounding decisions in verified evidence---offers a promising architectural approach for clinical AI systems. \textbf{Formal clinical validation is required before any claims about accuracy or safety can be made.}
...
\node[box, fill=mediumgray!30, draw=mediumgray, dashed] (wsi) at (0,0.3) {WSI\\Pathology$^\dagger$};
\node[box, fill=accentteal!10] (reconcile) at (3,0.3) {Evidence\\Reconciliation};

\draw[arrow] (imaging) -- (reconcile);
\draw[arrow, dashed, gray] (wsi) -- (reconcile);
...
% Legend for planned feature
\node[font=\scriptsize, text=mediumgray, align=left] at (5.5,-1.5) {$^\dagger$\textit{Planned for}\\~~\textit{next release}};

\end{tikzpicture}
\caption{Three-phase architecture of the Agentic Virtual Tumor Board. Phase 1 ensures data reliability through evaluator-optimizer loops. Phase 2 enforces adversarial deliberation with specialized critic and stewardship agents. Phase 3 grounds decisions in multimodal imaging evidence. \textit{Note: WSI Pathology integration (dashed box) is planned for a future release.}}
...
Our system indexes clinical guideline documents across 8 authoritative sources via Gemini File Search:
...
\begin{axis}[
    ...
    symbolic y coords={ClinVar, CIViC, SSO, ESMO, CAP, ASTRO, ACR, NCCN},
    ytick=data,
    nodes near coords,
...
\end{axis}
\end{tikzpicture}
\caption{Distribution of indexed guideline documents by source. NCCN provides the largest corpus (76 documents). SSO (Society of Surgical Oncology) provides 9 documents for surgical oncology guidance.}
...
\begin{itemize}[noitemsep]
    \item \textbf{Surgical Oncologist}: Primary SSO, secondary NCCN (context: 10,000 tokens)
...
The SSO corpus includes 9 open-access clinical recommendations:
\begin{itemize}[noitemsep]
    \item Breast-Conserving Surgery Margins (SSO-ASTRO 2014)
    \item Melanoma Gene Expression Profiling Consensus (2024)
    \item Choosing Wisely: 5 Things to Question (2021)
    \item Germline Testing in Breast Cancer (ASCO-SSO 2024)
    \item Partial Breast Irradiation Perspectives (2024)
\end{itemize}
...
\begin{tcolorbox}[colback=warnorange!10, colframe=warnorange, title=\textbf{Important: Preliminary Observations Only}]
\textbf{The metrics below are from informal developer testing on 10 synthetic cases and have NOT been validated through rigorous clinical evaluation.} These observations are presented to illustrate the system's intended behavior and evaluation framework---not as validated performance claims. Formal validation with oncologist review is required before any claims about clinical utility can be made.
\end{tcolorbox}

\subsubsection{Preliminary Observations (Unvalidated)}

\begin{table}[H]
\centering
\caption{Evaluation Framework and Preliminary Observations}
\label{tab:results}
\begin{tabular}{@{}lp{7cm}@{}}
\toprule
\textbf{Evaluation Dimension} & \textbf{Observation (Informal Testing)} \\
\midrule
...
\textit{Note: Formal validation methodology with oncologist adjudication is outlined in Section 5.2 but has not yet been executed. The 10 demo cases are synthetic and do not constitute a validation dataset.}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Stewardship} (after FISH+ confirmed): Surfaced biosimilar Trastuzumab option with estimated cost comparison (based on current Indian market prices)
\end{itemize}

\textbf{Observation}: In this synthetic case, the MARC-v1 loop behavior was observed to retry extraction when ``HER2 Equivocal'' was initially misclassified. The Stewardship agent surfaced biosimilar options with estimated cost comparisons. \textit{Note: The clinical appropriateness of these outputs has not been validated by oncologists.}

\subsubsection{Observed Failure Modes (Informal Testing)}

During informal developer testing, the following failure patterns were observed:

\begin{itemize}[noitemsep]
    \item \textbf{Rare variants}: Novel fusion partners sometimes not correctly interpreted
    \item \textbf{Conflicting guidelines}: Cases where NCCN and ESMO recommendations differed led to inconsistent outputs
    \item \textbf{Edge staging}: Ambiguous staging scenarios sometimes produced uncertain recommendations
\end{itemize}

The Scientific Critic was observed to flag some uncertain cases for human review. \textit{However, the completeness and reliability of this safety mechanism has not been formally evaluated.}

\subsection{Proposed Ablation Framework}

\begin{tcolorbox}[colback=warnorange!10, colframe=warnorange, title=\textbf{Note: Ablation Not Yet Conducted}]
The table below outlines the \textbf{proposed ablation study design}, not actual experimental results. Formal ablation studies are planned as part of future clinical validation work.
\end{tcolorbox}

\begin{table}[H]
\centering
\caption{Proposed Ablation Study Design (Not Yet Executed)}
\label{tab:ablation}
\begin{tabular}{@{}lp{8cm}@{}}
\toprule
\textbf{Configuration} & \textbf{Hypothesized Impact} \\
\midrule
...
Based on architectural design, we hypothesize that the MARC-v1 verification loop and Scientific Critic components are most critical for preventing downstream errors. \textbf{These hypotheses require formal testing to validate.}
...
Our transition from conversational AI to the Virtual Lab paradigm reflects a broader shift in medical AI design philosophy. By treating the tumor board as a \textit{scientific simulation} rather than a conversation, we \textbf{hypothesize} the following benefits (requiring validation):

\begin{enumerate}
    \item \textbf{Reduced Hallucination}: MARC-v1 loops are designed to prevent the system from inventing patient data
    \item \textbf{Safety-First Architecture}: Adversarial structure is intended to catch potentially dangerous recommendations
    \item \textbf{Economic Reality Integration}: Stewardship is designed to bring the India Context into clinical algorithms
    \item \textbf{Auditability}: The system is designed to trace recommendations to specific guideline citations
\end{enumerate}

\textit{Note: These design goals have not yet been empirically validated.}
...
\begin{tcolorbox}[
    ...
    colframe=warnorange,
    title={\textbf{Clinical Validation Status}},
    fonttitle=\bfseries,
    arc=3mm
]
\textbf{Important Disclaimer:} This work represents \textbf{early-stage conceptual research} and has not yet undergone rigorous clinical validation. The evaluation metrics reported are based on synthetic cases and internal assessments.

\vspace{0.5em}
\textbf{Not for Clinical Use:} The Agentic Virtual Tumor Board is a research prototype intended to demonstrate architectural concepts for multi-agent clinical decision support. It should \textbf{not} be used for actual patient care decisions without physician oversight and independent verification.

\vspace{0.5em}
\textbf{Seeking Clinical Partnerships:} We are actively seeking collaborations with oncology departments and cancer centers in India and internationally to conduct rigorous prospective clinical validation studies. Interested institutions are invited to contact the corresponding author to discuss partnership opportunities for:
\begin{itemize}[noitemsep, topsep=0pt]
    \item Retrospective case review studies comparing VTB recommendations with actual MTB decisions
    \item Prospective pilot studies with oncologist oversight and IRB approval
    \item Multi-site validation across diverse healthcare settings
\end{itemize}

\vspace{0.3em}
\noindent\textit{Contact: \texttt{spiff007@gmail.com} for collaboration inquiries.}
\end{tcolorbox}
...
\item Tang, X., et al. ``MedAgents: Large Language Models as Collaborators for Zero-shot Medical Reasoning.'' \textit{arXiv preprint arXiv:2311.10537}, 2023.

\item Wang, Z., et al. ``ColaCare: Enhancing Electronic Health Record Modeling through Large Language Model-Driven Multi-Agent Collaboration.'' \textit{arXiv preprint arXiv:2410.02551}, 2024.

\item Schmidgall, S., et al. ``AgentClinic: A Multimodal Agent Benchmark to Evaluate AI in Simulated Clinical Environments.'' \textit{arXiv preprint arXiv:2405.07960}, 2024.

\item Blondeel, M., et al. ``Healthcare Agent Orchestrator (HAO) for Patient Summarization in Molecular Tumor Boards.'' \textit{arXiv preprint arXiv:2509.06602}, 2025.

\item Garcia-Fernandez, C., et al. ``Trustworthy AI for Medicine: Continuous Hallucination Detection and Elimination with CHECK.'' \textit{arXiv preprint arXiv:2506.11129}, 2025.

\item Fanous, A., et al. ``SycEval: Evaluating LLM Sycophancy.'' \textit{arXiv preprint arXiv:2502.08177}, 2025.

\item Pan, J., et al. ``Beyond Benchmarks: Dynamic, Automatic and Systematic Red-Teaming Agents for Trustworthy Medical Language Models.'' \textit{arXiv preprint arXiv:2508.00923}, 2025.

\item Penn-RAIL. ``MARC-v1: Multi-Agent Reasoning and Coordination.'' 2026. \url{https://github.com/Penn-RAIL/MARC-v1}

\item Zheng, Q., et al. ``MIRIAD: Augmenting LLMs with Millions of Medical Query-Response Pairs.'' \textit{arXiv preprint arXiv:2506.06091}, 2025.

\item Sellergren, A., et al. ``MedGemma Technical Report.'' \textit{arXiv preprint arXiv:2507.05201}, 2025.

\item Hua, S., et al. ``PathFound: An Agentic Multimodal Model Activating Evidence-seeking Pathological Diagnosis.'' \textit{arXiv preprint arXiv:2512.23545}, 2025.

\item Palepu, A., et al. ``Exploring Large Language Models for Specialist-level Oncology Care.'' \textit{arXiv preprint arXiv:2411.03395}, 2024.

\item Mohammed, A.M., et al. ``Developing an Artificial Intelligence Tool for Personalized Breast Cancer Treatment Plans based on the NCCN Guidelines.'' \textit{arXiv preprint arXiv:2502.15698}, 2025.

\item Zakka, C., et al. ``Almanac: Retrieval-Augmented Language Models for Clinical Medicine.'' \textit{arXiv preprint arXiv:2303.01229}, 2023.

\item Ke, Y., et al. ``Development and Testing of Retrieval Augmented Generation in Large Language Models.'' \textit{arXiv preprint arXiv:2402.01733}, 2024.

\item Singhal, K., et al. ``Towards Expert-Level Medical Question Answering with Large Language Models.'' \textit{arXiv preprint arXiv:2305.09617}, 2023.

\item Zheng, H., et al. ``MedCoAct: Confidence-Aware Multi-Agent Collaboration for Complete Clinical Decision.'' \textit{arXiv preprint arXiv:2510.10461}, 2025.

\item Kim, Y., et al. ``Tiered Agentic Oversight: A Hierarchical Multi-Agent System for Healthcare Safety.'' \textit{arXiv preprint arXiv:2506.12482}, 2025.

\item Noori, A., et al. ``A Global Log for Medical AI.'' \textit{arXiv preprint arXiv:2510.04033}, 2025.

\item Menon, T.P., et al. ``Intersectionality of Cancer Disparities in South Asia.'' \textit{Lancet Global Health}, 2026; 14(2):e272-e280.

\item Debnath, S., et al. ``Epidemiological Profile of Patients with Malignant Neoplasm Admitted to a Tertiary Care Center in India.'' \textit{Frontiers in Oncology}, 2025; 15:1636807.
