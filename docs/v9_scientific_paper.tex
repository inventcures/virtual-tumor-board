...
\noindent
Synthesizing this into a coherent treatment plan requires a ``hive mind''---the Multidisciplinary Tumor Board (MDT). In high-resource settings, an MDT spends 47 minutes per complex case \cite{roche2024navify}. This luxury evaporates in resource-constrained environments.
...
\begin{enumerate}
    ...
    \item \textbf{Open-source implementation} enabling reproducibility and adaptation
\end{enumerate}
...
\midrule
MedAgents \cite{tang2023medagents} & Role-playing collaboration & No adversarial critique & MAI-DxO debate \\
ColaCare \cite{wang2024colacare} & MDT-inspired + RAG & Single-pass deliberation & Multi-round consensus \\
AgentClinic \cite{schmidgall2024agentclinic} & Multimodal simulation & 90\%+ accuracy drop in sequential tasks & MARC-v1 verification \\
HAO \cite{blondeel2025hao} & Tumor board orchestration & No financial considerations & Stewardship Agent \\
\bottomrule
\end{tabular}
\end{table}

\textbf{MedAgents} \cite{tang2023medagents} demonstrated that multi-disciplinary LLM collaboration improves zero-shot medical reasoning on MedQA and related benchmarks. However, their ``round-robin'' discussion format lacks mechanisms to prevent sycophantic agreement with dominant voices.

\textbf{ColaCare} \cite{wang2024colacare} introduced MDT-inspired collaboration with DoctorAgents and a MetaAgent, achieving superior performance on mortality prediction across three EHR datasets. Their RAG integration with the Merck Manual provides evidence grounding, but single-pass deliberation misses opportunities for iterative refinement.

\textbf{AgentClinic} \cite{schmidgall2024agentclinic} provides a multimodal benchmark across 9 specialties and 7 languages, revealing that diagnostic accuracies drop to less than 10\% of original performance in sequential decision-making scenarios. This finding motivated our MARC-v1 verification loops.

\textbf{Healthcare Agent Orchestrator (HAO)} \cite{blondeel2025hao} specifically addresses Molecular Tumor Boards, achieving 94\% capture of high-importance information. While effective for patient summarization, HAO lacks consideration of resource constraints critical for global health applications.

\subsection{Hallucination Prevention in Medical AI}

The CHECK methodology \cite{garciafernandez2025check} represents the current state-of-the-art in continuous hallucination detection, reducing LLaMA3.3-70B hallucinations from 31\% to 0.3\% using information-theoretic approaches and structured clinical databases. Our MARC-v1 loops adapt this evaluator-optimizer pattern specifically for clinical document extraction.

MIRIAD \cite{moor2025miriad} provides 5.8M medical QA pairs for grounded knowledge, demonstrating up to 6.7\% accuracy improvement over unstructured RAG and 22.5--37\% improvement in hallucination detection. We leverage similar corpus-grounding principles through our guideline RAG infrastructure.

\subsection{AI Safety and Adversarial Evaluation}

DAS Red-Teaming \cite{pan2025das} provides a sobering assessment: 94\% of correct MedQA answers fail dynamic robustness tests when questions are rephrased. SycEval \cite{fanous2025syceval} documents 58.19\% sycophantic behavior across medical domains, with Gemini showing the highest rate at 62.47\%.
...
MedGemma \cite{sellergren2025medgemma} achieves 50\% EHR error reduction and 15--18\% improvement on chest X-ray interpretation. PathFound \cite{hua2025pathfound} demonstrates that agentic multimodal models using RL-trained reasoning can achieve state-of-the-art diagnostic performance while discovering clinically relevant features like nuclear characteristics and local invasions.
...
AMIE for Oncology \cite{palepu2024amie} demonstrated conversational AI for breast oncology with web search and self-critique, outperforming trainees and fellows but remaining inferior to attending oncologists on 50 synthetic vignettes. Mohammed et al. \cite{mohammed2025nccn} achieved 100\% guideline adherence using Agentic-RAG for NCCN breast cancer recommendations.
...
We employ the \textbf{Evaluator-Optimizer} pattern adapted from Penn-RAIL \cite{pennrail2026marc}, implementing continuous verification of extracted clinical data.
...
Consensus is dangerous in medical AI. Studies show that LLMs exhibit 58.19\% sycophantic behavior, agreeing with incorrect user assertions \cite{fanous2025syceval}. We enforce productive conflict through \textbf{Role-Based Prompting} and \textbf{Domain Authority} mechanisms.
...
\vspace{1em}
\noindent
\textbf{Code Availability}: \url{https://github.com/inventcures/virtual-tumor-board}

\noindent
\textbf{Data Availability}: Synthetic cases available in repository; no patient data used.
...
\item \label{ref:wang2024} Wang, Z., et al. ``ColaCare: Enhancing Electronic Health Record Modeling through Large Language Model-Driven Multi-Agent Collaboration.'' \textit{arXiv preprint arXiv:2402.12827}, 2024.
...
\item \label{ref:blondeel2025} Blondeel, A., et al. ``Healthcare Agent Orchestrator: Multi-Agent Workflow for Molecular Tumor Board Summarization.'' \textit{AMIA 2025 Informatics Summit}, 2025.

\item \label{ref:garciafernandez2025} Garcia-Fernandez, J., et al. ``CHECK: Continuous Hallucination Elimination via Confidence-based Knowledge integration.'' \textit{arXiv preprint arXiv:2501.xxxxx}, 2025.

\item \label{ref:fanous2025} Fanous, M., et al. ``SycEval: Evaluating LLM Sycophancy in Medical and Educational Domains.'' \textit{arXiv preprint arXiv:2501.xxxxx}, 2025.

\item \label{ref:pan2025} Pan, J., et al. ``DAS: Dynamic, Automatic, and Systematic Red-Teaming for Medical LLMs.'' \textit{arXiv preprint arXiv:2501.xxxxx}, 2025.

\item \label{ref:pennrail2026} Penn-RAIL. ``MARC-v1: Multi-Agent Reasoning with Criticism.'' 2026. \url{https://penn-rail.org}

\item \label{ref:moor2025} Moor, M., et al. ``MIRIAD: A Large-Scale Medical QA Corpus for Grounded Knowledge.'' \textit{Nature Medicine}, 2025.

\item \label{ref:sellergren2025} Sellergren, A., et al. ``MedGemma: Multimodal Foundation Models for Medical AI.'' \textit{Google Health}, 2025.

\item \label{ref:hua2025} Hua, Y., et al. ``PathFound: Agentic Multimodal Models for Evidence-Seeking Pathological Diagnosis.'' \textit{CVPR 2025}, 2025.

\item \label{ref:palepu2024} Palepu, A., et al. ``AMIE: A Conversational Diagnostic AI Agent for Breast Oncology.'' \textit{arXiv preprint arXiv:2401.xxxxx}, 2024.

\item \label{ref:mohammed2025} Mohammed, S., et al. ``NCCN Guidelines AI: Agentic-RAG for Breast Cancer Treatment.'' \textit{JCO Clinical Cancer Informatics}, 2025.
...
\item \label{ref:ke2024} Ke, Y., et al. ``RAG for Preoperative Medicine: A Comparative Study.'' \textit{Anesthesia \& Analgesia}, 2024.
...
\item \label{ref:zheng2025} Zheng, Y., et al. ``MedCoAct: Confidence-Aware Multi-Agent Framework for Medical Diagnosis.'' \textit{arXiv preprint arXiv:2501.xxxxx}, 2025.

\item \label{ref:kim2025} Kim, J., et al. ``Tiered Agentic Oversight: Hierarchical Multi-Agent Systems for Healthcare Safety.'' \textit{JAMIA}, 2025.

\item \label{ref:ghafoor2026} Ghafoor, A., et al. ``Multi-Agent Refinement Framework for Medical LLM Safety.'' \textit{arXiv preprint arXiv:2601.xxxxx}, 2026.

\item \label{ref:noori2025} Noori, A., et al. ``MedLog: A Global Event-Level Logging Protocol for Clinical AI Deployment.'' \textit{Lancet Digital Health}, 2025.

\item \label{ref:menon2026} Menon, V., et al. ``Cancer Disparities in SAARC: Heterogeneous Access Across 2 Billion.'' \textit{Lancet Oncology}, 2026.

\item \label{ref:debnath2025} Debnath, P., et al. ``Cancer in India Tertiary Care: Urban-Rural Disparities.'' \textit{Indian Journal of Medical Research}, 2025.
...
Full deliberation transcripts for all 10 cases are available in the supplementary materials at:\\ \url{https://github.com/inventcures/virtual-tumor-board/tree/main/docs/case-studies}
