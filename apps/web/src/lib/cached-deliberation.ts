// Cached deliberation data for the demo case
// This avoids API costs while providing a realistic demo experience
...
export interface CachedAgentResponse {
  ...
  citations: string[];
  toolsUsed: string[];
  thinkingTime: number; // ms to simulate thinking
  typingSpeed: number; // chars per second for streaming
}
...
export interface CachedDeliberation {
  ...
  consensus: string;
  totalDuration: number; // expected total time in ms
}

export const DEMO_CASE_DELIBERATION: CachedDeliberation = {
  caseId: "demo-rajesh-kumar-nsclc",
  totalDuration: 45000, // 45 seconds total
  agents: [
    {
...
// Helper to get streaming chunks with realistic typing simulation
export function* streamDeliberationChunks(
  ...
  for (const agent of deliberation.agents) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      yield {
        ...
        delay: Math.floor((chunkSize / agent.typingSpeed) * 1000 * 0.3), // Faster for demo
      };
    }
  ...
  for (let i = 0; i < consensusChunks.length; i++) {
    yield {
      ...
      delay: 25, // Fast streaming for consensus
    };
  }
